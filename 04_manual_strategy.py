...
class Indicators():
    ...
    def bollinger_bands(self, prices, n=None, m=2):
    ...
    def momentum(self, prices, n=None):
        ...
        df_momentum['momentum_z'] = self.standard_score(df_momentum['momentum'])

        # print(df_momentum)
        return df_momentum['momentum_z']

    def volatility(self, prices, period=None):
        ...
        df_var['daily_return'] = self.compute_daily_returns(prices)
        df_var['volatility'] = df_var['daily_return'].rolling(window=period).std()

        # Standardized volatility using z-score
        df_var['volatility_z'] = self.standard_score(df_var['volatility'])
    ...
    def plot_indicators(self, df_indicators):
        ...
        df_bb_value.rename(columns={'bb_value': 'Bollinger Band BB Value'},
                           inplace=True)
        self.plot_data_with_line(df_bb_value, title="Bollinger Band BB Value", xlabel="Date", ylabel="BB Value",
        ...
        df_rsi.rename(columns={'RSI': 'Relative Strength Index (RSI)'},
                      inplace=True)
        self.plot_data_with_line(df_rsi, title="Relative Strength Index (RSI)", xlabel="Date", ylabel="RSI",
        ...
        df_momentum.rename(columns={'momentum': '14 days Momentum'},
                           inplace=True)
        self.plot_data_with_line(df_momentum, title="14 days Momentum", xlabel="Date", ylabel="Momentum",
        ...
        df_volatility.rename(columns={'volatility': '20 days Volatility'},
                             inplace=True)
        self.plot_data_with_line(df_volatility, title="20 days Volatility", xlabel="Date", ylabel="Volatility",
                            upper_y=1.0, lower_y=-1.0, middle_y=0.)
    ...
    def test_code(self):
        ...
        # Normalize data
        price_SPY = self.normalize(price_SPY)
        price_JPM = self.normalize(price_JPM)
        ...
        mv_avg = self.sma(price_JPM, rolling_window)
        # print(mv_avg)
        ...
        bb_value, upper_band, lower_band = self.bollinger_bands(price_JPM, n=rolling_window)
        ...
        rsi_value = self.rsi(price_JPM)
        # update df_indicators
        df_indicators['RSI'] = rsi_value
        # print(df_indicators)

        # 6. Get momentum
        momentum_value = self.momentum(price_JPM)
        df_indicators['momentum'] = momentum_value
        # print(df_indicators)

        # 7. Get volatility
        df_indicators['volatility'] = self.volatility(price_JPM)
        # print(df_indicators)

        # 8. Plot indicators
        self.plot_indicators(df_indicators)
